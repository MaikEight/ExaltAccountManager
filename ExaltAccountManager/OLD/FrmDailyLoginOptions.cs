...
namespace ExaltAccountManager
{
    public partial class FrmDailyLoginOptions : Form
    {
        ...
        public FrmDailyLoginOptions(FrmMainOLD _frm, NotificationOptions _opt)
        {
            InitializeComponent();
            frm = _frm;
            opt = _opt;
        ...
        private void btnRunTask_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    logins = (DailyLoginsOLD)frm.ByteArrayToObject(File.ReadAllBytes(frm.dailyLoginsPath));
                    logins.lastUpdate = new DateTime(2000, 1, 1);
                }
                else
                {
                    logins = new DailyLoginsOLD();
                    logins.logins = new System.Collections.Generic.List<DailyDataOLD>();
                }
