...
#pragma warning restore CS0612 
        ...
        public MK_EAM_Lib.AccountInfo GetAccountData(MK_EAM_Lib.AccountInfo info, bool getName = true)
        {
            try
            {
                ...
                try
                {
                    LogEvent(new LogData(logs.Count + 1, "EAM", LogEventType.WebRequest, $"Sending \"char/list\" for {info.email}."));
                    string link = $"https://www.realmofthemadgod.com/char/list?do_login=false&accessToken={WebUtility.UrlEncode(info.accessToken.token)}&game_net=Unity&play_platform=Unity&game_net_user_id=&muleDump=true&__source=jakcodex-v965";
        ...
        private void SaveAccountStats(MK_EAM_Lib.AccountInfo info, string request, string charList)
        {
            try
            {
                ...
                try
                {
                    AccountItems accountItems = new AccountItems(info.name, info.email, charList);
                    bool found = false;
                ...
                {
                    LogEvent(new LogData(logs.Count + 1, "EAM", LogEventType.EAMError, $"Failed to save AccountItems for {info.email}."));
                    optionenUI.BlinkLog((useDarkmode) ? Color.Crimson : Color.IndianRed);
                }
        ...
        public void ChangeDailyLoginState(MK_EAM_Lib.AccountInfo acc)
        {
            ...
            try
            {
                DailyLoginsOLD logins = null;
                try
                {
                    if (File.Exists(dailyLoginsPath))
                        logins = (DailyLoginsOLD)ByteArrayToObject(File.ReadAllBytes(dailyLoginsPath));
                }
                catch { }

                if (logins != null)
                {
                    List<DailyDataOLD> idsToRemove = new List<DailyDataOLD>();
                    ...
                    for (int i = 0; i < idsToAdd.Count; i++)
                        logins.logins.Add(new DailyDataOLD() { mail = accounts[idsToAdd[i]].email });
                }
                else
                {
                    logins = new DailyLoginsOLD();
                    for (int i = 0; i < accounts.Count; i++)
                    {
                        if (accounts[i].performSave)
                            logins.logins.Add(new DailyDataOLD() { mail = accounts[i].email });
                    }
                }
            ...
            try
            {
                if (File.Exists(exePath))
                {
                    if (accountInfo.accessToken != null)
                    {
                        ...
                        if (cTime.Date < DateTime.Now.Date || accountInfo.accessToken.validUntil < DateTime.Now || string.IsNullOrEmpty(accountInfo.accessToken.clientToken))
                        {
        ...
        private void scrollbar_ValueChanged(object sender, BunifuVScrollBar.ValueChangedEventArgs e)
        {
